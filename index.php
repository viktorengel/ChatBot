...
$hoy      = date('Y-m-d');
$ayer_tmp = date('Y-m-d', strtotime('-1 day'));
$dia_ayer = date('N', strtotime($ayer_tmp));
if ($dia_ayer == 7) $ayer_tmp = date('Y-m-d', strtotime('-3 days'));
if ($dia_ayer == 6) $ayer_tmp = date('Y-m-d', strtotime('-2 days'));
$ayer = $ayer_tmp;
...
var fechaRegistro = '<?= $hoy ?>';

function recargarEstudiantes(fecha) {
    fechaRegistro = fecha || '<?= $hoy ?>';

    // 1. Limpiar TODO inmediatamente antes de que llegue la respuesta
    document.getElementById('buscador-nombre').value = '';
    var listaNombre = document.getElementById('lista-nombre');
    if (listaNombre) { listaNombre.innerHTML = ''; listaNombre.style.display = 'none'; }

    // Limpiar seleccionados
    seleccionados = {};
    document.getElementById('hidden-estudiantes').innerHTML = '';
    document.getElementById('seleccionados-tags').innerHTML =
        '<span class="sin-seleccionados" id="sin-seleccionados">Ningún estudiante seleccionado aún</span>';
    actualizarContador();

    // 2. Ahora sí hacer el fetch — el buscador parte de cero
    fetch('estudiantes_ajax.php?fecha=' + encodeURIComponent(fechaRegistro))
        .then(r => r.json())
        .then(data => {
...
fetch('estudiantes_ajax.php?fecha=<?= $hoy ?>')
    .then(r => r.json())
